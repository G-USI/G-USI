...
class RayAddressField(StringField):
    ...
    - "http://ray.in-catalyst.com" - explicit cluster URL
...
class RayJobTarget(TargetGenerator):
    ...
    1. Create BUILD file:
        ...
        ray_job(
            ...
            num_cpus=2.0,
            memory=4 * 1024**3,  # 4GB
            env_vars={"USPTO_ZAI_API_KEY": None},
            dependencies=[
                "//src/python/catalyst/lib/commons",
                "//src/python/catalyst/lib/ai",
            ],
        )

    2. Package the job:
        pants package src/python/ictl/job/supsi/crawl_uspto:crawl_uspto

    3. Or submit directly:
        pants ray-submit src/python/ictl/job/supsi/crawl_uspto:crawl_uspto
