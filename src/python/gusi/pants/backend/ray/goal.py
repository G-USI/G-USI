        ...
        for dep_str in locked.get("requires_dists", []):
            ...
            if dep_req.marker:
                ...
                if "extra" in marker_text:
                    ...
                    if not any(
                        dep_req.marker.evaluate({**env, "extra": extra})
...
async def build_ray_job_outputs(request: RayJobBuildRequest) -> RayJobBuildOutputs:
    ...
    tt = await get_transitive_targets(
        TransitiveTargetsRequest((address,)), **implicitly()
    )
    ...
    relative = (
        spec_path[len(source_root.path) :].lstrip("/")
        if source_root.path
        else spec_path
    )
    ...
    unpinned_req_strings: tuple[str, ...] = ()
    if dep_addrs:
        job_tt = await get_transitive_targets(
            TransitiveTargetsRequest(dep_addrs), **implicitly()
        )
    ...
    if unpinned_req_strings:
        chosen_resolve = await choose_python_resolve(
            ChosenPythonResolveRequest(Addresses([address])), **implicitly()
        )
...
async def ray_submit(
    ...
    if ray_job_target is None:
        console.print_stderr(
            "No ray_job target found. Specify one on the command line."
        )
    ...
    reqs_digest = await create_digest(
        CreateDigest(
            [FileContent("requirements.txt", outputs.requirements_content.encode())]
        ),
        **implicitly(),
    )
...
async def package_ray_job(field_set: RayJobPackageFieldSet) -> BuiltPackage:
    ...
    reqs_digest = await create_digest(
        CreateDigest(
            [FileContent(reqs_filename, outputs.requirements_content.encode())]
        ),
        **implicitly(),
    )
